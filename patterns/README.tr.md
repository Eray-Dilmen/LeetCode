...
* **Karmaşıklık Optimizasyonu:** Kaba kuvvet (Brute Force) çözümünden (örn. `O(n²)`), en uygun veri yapısı veya algoritmik kalıbı kullanarak optimal sonuca (örn. `O(n)` veya `O(log n)`) geçiş adımlarını standartlaştırır.
...
* **`O(1)` arama hızına veya frekans sayımına mı ihtiyacım var?** -> [01-hash-maps-and-sets](./01-hash-maps-and-sets/)
* **Dizi sıralanmış mı (sorted)?** -> [02-two-pointers](./02-two-pointers/) veya [06-binary-search](./06-binary-search/)
* **Bitişik bir alt dizi (subarray) veya alt metin mi arıyorum?** -> [03-sliding-window](./03-sliding-window/)
* **Alt dizilerin toplamlarını birden fazla kez mi sorguluyorum?** -> [04-prefix-sum](./04-prefix-sum/)
* **Bir "sonraki daha büyük/küçük" (next greater) elemanı mı bulmam gerekiyor?** -> [07-monotonic-stack](./07-monotonic-stack/)
* **Çakışan zaman aralıklarını/menzilleri mi birleştiriyorum?** -> [08-intervals](./08-intervals/)

### 2. Bağlı Listeler (Linked Lists)
Problem ardışık olarak birbirini gösteren düğümler içeriyorsa:
* **`O(1)` alan kullanarak döngü, orta nokta veya kopya kontrolü mü yapıyorum?** -> [05-fast-and-slow-pointers](./05-fast-and-slow-pointers/)

### 3. Ağaçlar (Trees) ve Çizgeler (Graphs)
Problem hiyerarşik veri, ağlar veya matrisler (grid) içeriyorsa:
* **Kökten yaprağa (root-to-leaf) yolları mı inceliyorum veya BST'yi mi doğruluyorum?** -> [09-tree-dfs](./09-tree-dfs/)
* **En kısa yolu (shortest path) veya seviye bazlı verileri mi arıyorum?** -> [10-tree-bfs](./10-tree-bfs/)
* **Ortada bir bağımlılık/önkoşul zinciri mi var?** -> [11-graphs-and-topological-sort](./11-graphs-and-topological-sort/)
* **Devasa boyutlarda metin ön ek (prefix) araması mı yapıyorum?** -> [16-trie](./16-trie/)

### 4. Arama, Optimizasyon ve Kombinasyon
Problem "en iyi", "tüm olası" veya "en verimli" yolu soruyorsa:
* **En büyük/küçük K (Top K) elemanlara mı ihtiyacım var?** -> [12-heap-and-top-k-elements](./12-heap-and-top-k-elements/)
* **TÜM olası kombinasyonları/permütasyonları mı istiyor?** -> [13-backtracking](./13-backtracking/)
* **Tekrar eden alt problemlerle (overlapping) min/maks/toplam yol sayısını mı soruyor?** -> [14-dynamic-programming](./14-dynamic-programming/)
* **Sadece yerel (local) en iyiyi seçerek genel (global) optimuma ulaşabiliyor muyum?** -> [15-greedy-algorithms](./15-greedy-algorithms/)

### 5. Matematik ve Donanım Seviyesi
* **Aşırı hızlı, `O(1)` alan kullanan mantıksal bayraklara veya XOR mantığına mı ihtiyacım var?** -> [17-bit-manipulation](./17-bit-manipulation/)
